...
router.get("/", isLoggedIn, (req, res) =>{
  ...
  db.query(query1, [req.user.id, req.user.id], (err, results)=> {
    ...
    db.query("select * from posts inner join users on posts.user_id = users.id order by post_id desc;", function(err, result) {
      // console.log(result[0]);
      res.render("../views/home", {allposts : result, friend_count: friend_count});
    });
});
